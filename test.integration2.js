...
const { generateTeams } = require('./src/lib/teamActions.ts');
const fs = require('fs');
...
async function runTests() {
  ...
  const backupPath = path.join(process.cwd(), 'volleyball.db.backup');
  fs.copyFileSync(dbPath, backupPath);
  console.log('Backed up database to', backupPath);

  try {
    // 1. Test participant import
    let participants = await db.getParticipants();
    ...
    for (const p of testParticipants) {
      for (const e of evaluators) {
        await db.deleteEvaluation(p.id, e);
      }
    }
    ...
    for (let i = 0; i < testParticipants.length; i++) {
      ...
      for (let j = 0; j < evaluators.length; j++) {
        ...
        const result = await db.createEvaluation(evalData);
    ...
    for (const p of testParticipants) {
      const evals = await db.getEvaluationsByParticipantId(p.id);
      ...
      for (const e of evaluators) {
        const found = evals.some(ev => ev.evaluatorName === e);
        if (!found) {
    ...
    for (const p of testParticipants) {
      const consensus = await db.getConsensusForParticipant(p.id);
      if (!consensus) {
        console.error(`ERROR: No consensus found for participant ${p.id}`);
        process.exit(1);
      }
      // Find the evaluations for this participant from our test data
      const participantEvals = testData.filter(d => d.participant.id === p.id);
      // Calculate expected level (mode)
      const levels = participantEvals.map(d => d.level);
      const levelCounts = {};
      let maxCount = 0;
      let modeLevel = null;
      ...
      const avgRanking = participantEvals.reduce((sum, d) => sum + d.ranking, 0) / participantEvals.length;
    ...
    await db.deleteEvaluation(pEdge.id, 'Douglas');
    const evalsAfter = await db.getEvaluationsByParticipantId(pEdge.id);
    if (evalsAfter.length !== 2) {
      console.error(`ERROR: After deletion, expected 2 evaluations, got ${evalsAfter.length}`);
      process.exit(1);
    }
    await db.recalculateConsensusFromEvaluations();
    const consensusAfter = await db.getConsensusForParticipant(pEdge.id);
    if (!consensusAfter) {
