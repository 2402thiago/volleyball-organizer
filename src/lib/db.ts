...
async function recalculateConsensusFromEvaluations() {
  ...
  const participantEvals: Record<string, { levels: string[]; rankings: number[] }> = {};
  evaluations.forEach(evalRecord => {
    const pid = evalRecord.participantId;
    if (!participantEvals[pid]) {
      participantEvals[pid] = { levels: [], rankings: [] };
    }
    participantEvals[pid].levels.push(evalRecord.level);
    participantEvals[pid].rankings.push(evalRecord.ranking);
  });
